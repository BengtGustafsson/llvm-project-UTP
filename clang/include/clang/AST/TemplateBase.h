...
public:
  ...
  enum ArgKind {
    ...
    /// The template argument is an universal template parameter.
    Universal,
...
private:
  ...
  union {
    struct DA DeclArg;                      // Declaration
    struct I IntArg;                        // Integral
    struct TA TemplateArg;                  // Template / TemplateExpansion
    Expr* ExprArg;                          // Expression
    struct A PackArgs;                      // Pack
    UniversalTemplateParmDecl* UnivArg;     // Universal
  };

  QualType TypeArg;
  ArgKind Kind = Null;      // Default is Invalid

public:
  /// Construct an empty, invalid template argument.
  constexpr TemplateArgument() = default;
  ...
  llvm::APSInt getAsIntegral() const {
    ...
    return APSInt(APInt(IntArg.BitWidth, makeArrayRef(IntArg.pVal, NumWords)),
                  IntArg.IsUnsigned);
  }
  ...
  ArrayRef<TemplateArgument> getPackAsArray() const {
    assert(getKind() == Pack);
    return llvm::makeArrayRef(PackArgs.Args, PackArgs.NumArgs);
  }
